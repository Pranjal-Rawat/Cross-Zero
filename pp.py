...
def delete():
    if 'prod_box' in globals():
        prod_box.destroy()
    if 'delete_btn' in globals():
        delete_btn.destroy()
...
file_menu.add_command(label="Customer")
file_menu.add_command(label="Orders")
file_menu.add_separator()
file_menu.add_command(label="Exit")

file_options = Menu(menu, tearoff=0)
file_options.add_command(label="Empty Cart", command=empty)
file_options.add_command(label="O - 2")
file_options.add_command(label="O - 3")
file_options.add_separator()
file_options.add_command(label="Exit")
...
# Functionality to increase quantity
def add(prod_id):
...
# Functionality to decrease quantity
def sub(prod_id):
...
# Functionality to place an order
order_count = 0


def order(price):
    global order_count
    confirmed = messagebox.askyesno("Confirm Order", f"Place order for ${price}?")
    ...
    if confirmed:
        ...
        for prod_id, details in cart_items.items():
            file.write(f"{current_date},{order_count},{address},{details['name']},{details['quantity']},{details['price']}\n")
...
shop_frame.bind("<Configure>", lambda e: canvas_shop.configure(scrollregion=canvas_shop.bbox("all")))
